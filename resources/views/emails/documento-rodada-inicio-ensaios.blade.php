...
<body
    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div
            ...
            <p>
                Informamos que a data de <strong>início dos ensaios</strong> da Rodada
                <strong>{{ $rodada->descricao }}</strong> do Programa de Ensaio de Proficiência (PEP)
                <strong>{{ $rodada->agendainterlab->interlab->nome }}</strong> chegou.
                Os ensaios devem ser iniciados a partir de hoje.
            </p>

            @if ($rodada->descricao_arquivo_inicio_ensaios)
            <div style="background-color: #FFF3CD; padding: 12px; border-radius: 3px; margin-bottom: 10px; color: #856404;">
                <p><strong>Informações adicionais:</strong> <br>
                    {!! nl2br(e($rodada->descricao_arquivo_inicio_ensaios)) !!}
                </p>
            </div>
            @endif

            @if ($rodada->arquivo_inicio_ensaios)
                <p>
                    <a href="{{ url('interlab-material/' . $rodada->arquivo_inicio_ensaios) }}" target="_blank"
