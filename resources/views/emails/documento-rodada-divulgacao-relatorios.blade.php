...
<body
    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div
            ...
            <p>
                Informamos que os <strong>relatórios</strong> da Rodada
                <strong>{{ $rodada->descricao }}</strong> do Programa de Ensaio de Proficiência (PEP)
                <strong>{{ $rodada->agendainterlab->interlab->nome }}</strong> estão disponíveis para consulta.
            </p>

            @if ($rodada->descricao_arquivo_divulgacao_relatorios)
                <div style="background-color: #FFF3CD; padding: 12px; border-radius: 3px; margin-bottom: 10px; color: #856404;">
                    <p><strong>Informações adicionais:</strong> <br>
                        {!! nl2br(e($rodada->descricao_arquivo_divulgacao_relatorios)) !!}
                    </p>
                </div>
            @endif

            @if ($rodada->arquivo_divulgacao_relatorios)
                <p>
                    <a href="{{ url('interlab-material/' . $rodada->arquivo_divulgacao_relatorios) }}" target="_blank"
