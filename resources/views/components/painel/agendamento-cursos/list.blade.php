    ...
    <div class="table-responsive" style="min-height: 25vh">
      <table class="table table-responsive table-striped align-middle mb-0">
        ...
        <tbody>
          ...
          <tr>
            <th class="text-uppercase">{{ Carbon\Carbon::parse($agendacurso->data_inicio)->locale('pt-BR')->translatedFormat('F') }}</th>
            <td class="text-center">{{ $agendacurso->id }}</td>
            <td @if ($agendacurso->status == 'CONFIRMADO')
              class="text-success fw-bold"
              @elseif ($agendacurso->status == 'REAGENDAR')
              class="text-primary fw-bold"
              @elseif ($agendacurso->status == 'CANCELADO')
              class="text-danger fw-bold"
              @endif >{{ $agendacurso->status }}</td>
            <td style="white-space: nowrap; margin: 10px 0 10px">{{ Carbon\Carbon::parse($agendacurso->data_inicio)->format('d-m-Y') }}</td>
            ...
            <td>
              <div class="dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink2">
                  <li><a class="dropdown-item" href="{{ route('agendamento-curso-insert', $agendacurso->uid) }}">Editar</a>
                  </li>
                  <li>
                    <form method="POST" action="{{ route('agendamento-curso-delete', $agendacurso->uid) }}">
                      @csrf
